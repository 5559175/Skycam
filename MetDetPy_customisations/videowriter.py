...
class FFMpegVideoWriter(BaseVideoWriter):
    ...
    def save_video(cls,
        ...
        try:
            ...
            ff_cmd: list[str] = [
                config.ffmpeg_path, '-i', tmpf, '-c', 'copy', '-pix_fmt', config.pix_fmt, '-an', '-y',
                video_path
            ]
            logger.info(
    ...
    def save_video_by_stream(cls,
        ...
        ff_cmd: list[str] = [
            ...
            f"{(S - K):.3f}", '-t', f"{(E - S):.3f}", '-c', 'copy',
            '-pix_fmt', config.pix_fmt, *audio_args,
    ...
    def save_video_with_audio(cls,
        ...
        try:
            ...
            ff_cmd = [
                ...
                '1:a:0?', '-c', 'copy', '-pix_fmt', config.pix_fmt, *audio_args,
                '-avoid_negative_ts', '1', '-y', video_path
            ]
